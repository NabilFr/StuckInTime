...
public class TimeController {
    ...
    private String heroState(int heroIndex) {

        switch (personnage1.getHeroState(heroIndex)) {
            case 1 :
                return "hidden";
            default:
                return "visible";
    ...
    public String chooseYourHero(Model model, @RequestParam(name = "username", required=false, defaultValue = "") String name) {
        ...
        model.addAttribute("state1", heroState(0));
        model.addAttribute("state2", heroState(1));
        model.addAttribute("state3", heroState(2));
        model.addAttribute("state4", heroState(3));
        model.addAttribute("state5", heroState(4));
        model.addAttribute("state6", heroState(5));
    ...
    public String weaponChoice(Model model, @RequestParam(name = "weapon") int weapon) {
        ...
        model.addAttribute("score", personnage1.getScore());
        String attack1;
        String attack2;
        if (weapon == 0) {
    ...
    public String attackChoices(Model model, @RequestParam(name = "attack") int attack) {
        ...
        switch (Arrays.toString(scenario)) {
            ...
            case "[3, 0, 0]":
                ...
                break;
            case "[3, 0, 1]":
                ending = String.format("Le Mask fait quelques étirements, craque sa nuque, puis, avec de la peinture de guerre étalée sur les joues, il court en hurlant un cri de guerre en direction du monstre... Sauf qu'il trébuche sur une racine et se la plante dans l’œil. C'était bien tenté. Quel dommage qu'il y ait eu cette racine au milieu de la prairie... Ah oui et pendant ce temps, le géant zigouille %s.", personnage1.getName());
                score = -5;
                personnage1.setScore(score);
                break;
            case "[3, 1, 0]":
